...
func (r *Rest) parseRateLimitHeaders(header http.Header) (rateLimitHeaders, bool) {
	var err error
...
func (r *Rest) createLimiter(headers rateLimitHeaders, routeName string) {
	...
	prefillloop:
		...
		for i := 0; i < headers.Remaining; i++ {
			...
			default:
				log.Warn("rate limiting channel was too small; you should increase the default capacity", "remaining", headers.Remaining)
				break prefillloop
			}
		}
		...
		go func() {
			...
			refillloop:
				...
				for i := 0; i < refill.maxRequests; i++ {
					...
					case limiter.requests <- struct{}{}:
					default:
						log.Warn("rate limiting channel was too small; you should increase the default capacity", "maxRequests", refill.maxRequests)
						break refillloop
					}
				}
...
func (r *Rest) doWithRateLimiting(ctx context.Context, routeName string, getReq func(ctx context.Context) *http.Request) (*http.Response, error) {
	var bucket string
	...
	for {
		...
		headers, headersOk := r.parseRateLimitHeaders(res.Header)
		if headersOk {
			if limiter == nil || headers.Bucket != bucket {
				r.createLimiter(headers, routeName)
			} else {
